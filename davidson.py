...
class DavidsonManager:
    ...
    def mgs(self, n, V_ik, t_ik):
        ...
        t_k = np.zeros(n)
        self.comm.Allgather([t_ik, MPI.DOUBLE], [t_k, MPI.DOUBLE])
    ...
    def distributed(self, A_i, n, n_eig, eps, max_iter, m, q):
        ...
        """
        self.local_size = n // self.world_size
        assert self.local_size * self.world_size == n
        ...
        for j in range(n_eig):
            for k in range(1, max_iter):
                ...
                self.comm.Allgather([local_vik, MPI.DOUBLE], [V_k, MPI.DOUBLE])
                ...
                r_k = np.zeros(n)
                self.comm.Allgather([r_ik, MPI.DOUBLE], [r_k, MPI.DOUBLE])
...
if __name__ == "__main__":
    ...
    A_i = np.zeros((n // DM.world_size, n))
    A_i = 0.0001 * np.random.randn(n // DM.world_size, n)

    L_k = DM.distributed(A_i, n, n_eig, epsilon, max_iter, m, q)

    # fuse A across all nodes
    A = np.zeros((n, n))
    DM.Allgather([A_i, MPI.DOUBLE], [A, MPI.DOUBLE])
